...
export default function Reports() {
  ...
  const CustomTooltip = ({active,payload,label})=>{
    ...
    return(
      <div style={{background:"#fff",border:"1px solid rgba(0,0,0,0.09)",borderRadius:10,padding:"10px 14px",boxShadow:"0 4px 20px rgba(0,0,0,0.1)",fontFamily:"'DM Sans',sans-serif"}}>
        <div style={{fontSize:11,fontWeight:700,color:"#6e6e73",marginBottom:5}}>{label}</div>
        {payload.map((p,i)=><div key={i} style={{fontSize:13,fontWeight:700,color:p.color||"#0a0a0a"}}>{p.name}: {p.value}</div>)}
      </div>
    );
  };
  ...
  return (
    <>
      <style>{`
        .rp-root { min-height:100vh; background:#f5f5f7; padding-top:60px; padding-left:200px; font-family:'DM Sans',sans-serif; }
        ...
        .rp-title { font-size:28px;font-weight:800;color:#0a0a0a;letter-spacing:-.5px;margin:0 0 4px; }
        .rp-sub   { font-size:13px;color:#6e6e73;margin:0 0 28px; }
        ...
        .rp-stat:hover { transform:translateY(-2px); box-shadow:0 6px 20px rgba(0,0,0,0.09); }
        .rp-stat-bar   { position:absolute;top:0;left:0;right:0;height:3px;border-radius:14px 14px 0 0; }
        .rp-stat-lbl   { font-size:9px;font-weight:800;color:#a1a1a6;text-transform:uppercase;letter-spacing:1.2px;margin-bottom:8px; }
        .rp-stat-val   { font-size:28px;font-weight:800;color:#0a0a0a;letter-spacing:-1px;line-height:1; }

        /* SECTION */
        .rp-sec-title { font-size:18px;font-weight:800;color:#0a0a0a;letter-spacing:-.3px;margin-bottom:16px; }
        ...
        .rp-chart-title { font-size:13px;font-weight:700;color:#0a0a0a;margin-bottom:18px;display:flex;align-items:center;gap:7px; }
        .rp-chart-title span { font-size:15px; }

        /* TREND full width */
        .rp-trend-card {
          background:#fff; border:1px solid rgba(0,0,0,0.08); border-radius:16px;
          padding:20px 22px; box-shadow:0 1px 4px rgba(0,0,0,0.06); margin-bottom:28px;
        }

        /* PIE legend */
        .rp-pie-legend { display:flex;flex-direction:column;gap:8px;justify-content:center; }
        .rp-pie-legend-item { display:flex;align-items:center;gap:8px;font-size:12px;color:#3d3d3d;font-weight:600; }
        ...
        .rp-sum-title { font-size:12px;font-weight:700;color:#a1a1a6;text-transform:uppercase;letter-spacing:1px;margin-bottom:14px; }
        .rp-sum-row   { display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid rgba(0,0,0,0.05); }
        .rp-sum-row:last-child { border-bottom:none; }
        .rp-sum-k { font-size:12px;color:#6e6e73;font-weight:500; }
        .rp-sum-v { font-size:13px;font-weight:800;color:#0a0a0a; }

        /* Recharts overrides */
        .recharts-cartesian-axis-tick-value { fill:#a1a1a6!important; font-size:11px!important; font-family:'DM Sans',sans-serif!important; }
        .recharts-cartesian-grid-horizontal line, .recharts-cartesian-grid-vertical line { stroke:rgba(0,0,0,0.06)!important; }
      ...
      <div className="rp-root">
        <div className="rp-inner">
          ...
          {/* OVERVIEW STATS */}
          <div style={{fontSize:15,fontWeight:800,color:"#0a0a0a",marginBottom:14,letterSpacing:"-.3px"}}>Overview</div>
          ...
          {/* BREAKDOWN */}
          <div style={{fontSize:15,fontWeight:800,color:"#0a0a0a",marginBottom:14,letterSpacing:"-.3px"}}>Breakdown</div>
          ...
          <div className="rp-charts">
            ...
            <div className="rp-chart-card">
              ...
              {pieData.length>0 ? (
                <div style={{display:"flex",alignItems:"center",gap:24}}>
                  <ResponsiveContainer width={180} height={180}>
                    <PieChart>
                      ...
                      <Tooltip contentStyle={{background:"#fff",border:"1px solid rgba(0,0,0,0.09)",borderRadius:10,fontFamily:"'DM Sans',sans-serif",fontSize:12}}/>
                    </PieChart>
                  </ResponsiveContainer>
                ...
                <div style={{height:180,display:"flex",alignItems:"center",justifyContent:"center",color:"#a1a1a6",fontSize:13}}>No data yet</div>
              )}
            </div>
            ...
            <div className="rp-trend-card">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={trendData} margin={{top:0,right:16,left:-20,bottom:0}}>
                  ...
                  <YAxis allowDecimals={false} tick={{fill:"#a1a1a6",fontSize:11,fontFamily:"'DM Sans',sans-serif"}} axisLine={false} tickLine={false}/>
                  <Tooltip content={<CustomTooltip/>}/>
                  <Legend wrapperStyle={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#3d3d3d"}}/>
          ...
          {/* SUMMARY CARDS */}
          <div style={{fontSize:15,fontWeight:800,color:"#0a0a0a",marginBottom:14,letterSpacing:"-.3px"}}>Fleet &amp; Hospital Summary</div>
