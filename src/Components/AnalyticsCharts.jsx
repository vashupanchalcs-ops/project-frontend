...
export default function AnalyticsCharts() {
  ...
  useEffect(() => {
    fetch(`${(import.meta.env.VITE_API_BASE_URL || (import.meta.env.DEV ? "http://127.0.0.1:8000" : "https://swiftrescue-backend-shlb.onrender.com")).replace(/\/+$/, "")}/api/bookings/`).then(r=>r.json()).then(setBookings).catch(()=>{});
    fetch(`${(import.meta.env.VITE_API_BASE_URL || (import.meta.env.DEV ? "http://127.0.0.1:8000" : "https://swiftrescue-backend-shlb.onrender.com")).replace(/\/+$/, "")}/api/ambulances/`).then(r=>r.json()).then(setAmbulances).catch(()=>{});
    const t = setInterval(()=>{
      fetch(`${(import.meta.env.VITE_API_BASE_URL || (import.meta.env.DEV ? "http://127.0.0.1:8000" : "https://swiftrescue-backend-shlb.onrender.com")).replace(/\/+$/, "")}/api/bookings/`).then(r=>r.json()).then(setBookings).catch(()=>{});
    }, 15000);
