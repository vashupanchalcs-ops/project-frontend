...
const DriverLocationTracker = ({ ambulanceId, driverEmail, bookingId }) => {
  ...
  useEffect(() => {
    ...
    const sendLocation = (latitude, longitude, accuracy, speed) => {
      ...
      // Send to backend
      fetch(`${(import.meta.env.VITE_API_BASE_URL || (import.meta.env.DEV ? "http://127.0.0.1:8000" : "https://swiftrescue-backend-shlb.onrender.com")).replace(/\/+$/, "")}/api/driver-location/`, {
        method: 'POST',
