...
export default function DriverVoiceReports() {
  ...
  const upload = async () => {
    const pending = activeId ? pendingByBooking[activeId] : null;
    const drafts = activeId
      ? [...(draftsByBooking[activeId] || []), ...(pending ? [pending] : [])]
      : [];
    ...
    try {
      ...
      setPendingByBooking((current) => ({ ...current, [activeId]: null }));
      setBookings((current) => current.map((row) => (
        Number(row.id) === Number(activeId)
          ? { ...row, condition_photos: [...(Array.isArray(row.condition_photos) ? row.condition_photos : []), ...(Array.isArray(data.photos) ? data.photos : [])] }
          : row
      )));
