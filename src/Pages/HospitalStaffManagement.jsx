    ...
    <div className="hsm-page">
      ...
      {directoryOnly && (
        <nav className="hsm-directory-tabs" aria-label="Employee views">
          {[["employees", "Employees"], ["departments", "Departments"], ["positions", "Positions"], ["performance", "Performance Review"]].map(([key, label]) => (
...
function StaffDrawer({ form, drawer, saving, error, onChange, onSubmit, onClose }) {
  ...
  return <div className="hsm-drawer-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="hsm-drawer" role="dialog" aria-modal="true" aria-label={isEdit ? "Edit staff" : "Add staff"}>
      ...
      <form className="hsm-form" onSubmit={onSubmit}>
        {error && <div className="hsm-alert hsm-alert-error">{error}</div>}
        ...
        <div className="hsm-drawer-foot"><button type="button" className="hsm-btn hsm-btn-light" onClick={onClose} disabled={saving}>Cancel</button><button type="submit" className="hsm-btn hsm-btn-primary" disabled={saving}>{saving ? "Saving…" : isEdit ? "Save changes" : "Add staff"}</button></div>
      </form>
    </aside>
  </div>;
}
